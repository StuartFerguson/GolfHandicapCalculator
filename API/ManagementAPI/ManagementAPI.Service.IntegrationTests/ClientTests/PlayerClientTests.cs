namespace ManagementAPI.Service.IntegrationTests.ClientTests
{
    ...
    public class PlayerClientTests : IClassFixture<ManagmentApiWebFactory<Startup>>
    {
        ...
        [Fact]
        public async Task PlayerClient_GetPlayerMemberships_MembershipsReturned()
        {
            // 1. Arrange
            HttpClient client = this.WebApplicationFactory.AddPlayer().CreateClient();
            Func<String, String> resolver = api => "http://localhost";
        ...
        [Fact]
        public async Task PlayerClient_GetTournamentsSignedUpFor_TournamentsReturned()
        {
            // 1. Arrange
            HttpClient client = this.WebApplicationFactory.AddPlayer().CreateClient();
            Func<String, String> resolver = api => "http://localhost";
        ...
        [Fact]
        public async Task PlayerClient_SignInForTournament_PlayerSignedIn()
        {
            // 1. Arrange
            HttpClient client = this.WebApplicationFactory.CreateClient();
            Func<String, String> resolver = api => "http://localhost";
            ...
            // 2. Act
            await playerClient.SignInForTournament(token, TestData.PlayerId, TestData.TournamentId, CancellationToken.None);
        }

        [Fact]
        public async Task PlayerClient_RecordPlayerScore_ScoreRecorded()
        {
            // 1. Arrange
            HttpClient client = this.WebApplicationFactory.CreateClient();
            Func<String, String> resolver = api => "http://localhost";
            ...
            // 2. Act
            await playerClient.RecordPlayerScore(token, TestData.PlayerId, TestData.TournamentId, TestData.RecordPlayerTournamentScoreRequest, CancellationToken.None);
        }
    }
}