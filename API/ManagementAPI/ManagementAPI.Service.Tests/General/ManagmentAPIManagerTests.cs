namespace ManagementAPI.Service.Tests.General
{
    ...
    public class ManagmentAPIManagerTests
    {
        ...
        [Fact]
        public async Task ManagementAPIManager_GetPlayerSignedUpTournaments_ListReturned()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            PlayerSignedUpTournamentsResponse result = await manager.GetPlayerSignedUpTournaments(PlayerTestData.AggregateId, CancellationToken.None);

            result.ShouldNotBeNull();
            result.PlayerSignedUpTournaments.ShouldNotBeNull();
            result.PlayerSignedUpTournaments.Count.ShouldBe(1);
        }

        [Fact]
        public async Task ManagementAPIManager_GetPlayerSignedUpTournaments_NoSignedUpTournaments_EmptyListReturned()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            result.ShouldNotBeNull();
            result.PlayerSignedUpTournaments.ShouldNotBeNull();
            result.PlayerSignedUpTournaments.ShouldBeEmpty();
        }

        [Fact]
        public async Task ManagementAPIManager_GetPlayerSignedUpTournaments_NoSignedUpTournamentsAwaitingScoreEntry_EmptyListReturned()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
        ...
        [Fact]
        public async Task ManagementAPIManager_InsertMeasuredCourseToReadModel_DuplicateRecord_NoErrorThrown()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            context.MeasuredCourses.Add(new MeasuredCourse
                                        {
                                            GolfClubId = GolfClubTestData.AggregateId,
                                            ...
                                            SSS = GolfClubTestData.StandardScratchScore
                                        });
            ...
            MeasuredCourseAddedEvent domainEvent = GolfClubTestData.GetMeasuredCourseAddedEvent();

            Should.NotThrow(async () => { await manager.InsertMeasuredCourseToReadModel(domainEvent, CancellationToken.None); });
        }

        [Fact]
        public async Task ManagementAPIManager_InsertMeasuredCourseToReadModel_GolfClubNotFound_ErrorThrown()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            Should.Throw<NotFoundException>(async () => { await manager.InsertMeasuredCourseToReadModel(domainEvent, CancellationToken.None); });
        }

        [Fact]
        public async Task ManagementAPIManager_InsertMeasuredCourseToReadModel_NullEvent_ErrorThrown()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            MeasuredCourseAddedEvent domainEvent = null;

            Should.Throw<ArgumentNullException>(async () => { await manager.InsertMeasuredCourseToReadModel(domainEvent, CancellationToken.None); });
        }

        [Fact]
        public async Task ManagementAPIManager_InsertMeasuredCourseToReadModel_RecordInsertedSuccessfully()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            await manager.InsertMeasuredCourseToReadModel(domainEvent, CancellationToken.None);

            ManagementAPIReadModel verifyContext = this.GetContext(databaseName);
            verifyContext.MeasuredCourses.Count().ShouldBe(1);
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerHandicapRecordToReporting_GolfClubNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.InsertPlayerHandicapRecordToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerHandicapRecordToReporting_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.InsertPlayerHandicapRecordToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerHandicapRecordToReporting_RecordInsertedSuccessfully()
        {
            ...
            Should.NotThrow(async () => { await manager.InsertPlayerHandicapRecordToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerMembershipToReadModel_ClubMembershipRequestAcceptedEvent_DuplicateRecord_NoErrorThrown()
        {
            ...
            Should.NotThrow(async () => { await manager.InsertPlayerMembershipToReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerMembershipToReadModel_ClubMembershipRequestAcceptedEvent_GolfClubNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.InsertPlayerMembershipToReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerMembershipToReadModel_ClubMembershipRequestAcceptedEvent_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.InsertPlayerMembershipToReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerMembershipToReporting_GolfClubNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.InsertPlayerMembershipToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerMembershipToReporting_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.InsertPlayerMembershipToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertPlayerMembershipToReporting_RecordInsertedSuccessfully()
        {
            ...
            Should.NotThrow(async () => { await manager.InsertPlayerMembershipToReporting(domainEvent, CancellationToken.None); });
        }

        [Fact]
        public async Task ManagementAPIManager_InsertPlayerScoreToReadModel_NullEvent_ErrorThrown()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            PlayerScorePublishedEvent domainEvent = null;

            Should.Throw<ArgumentNullException>(async () => { await manager.InsertPlayerScoreToReadModel(domainEvent, CancellationToken.None); });
        }

        [Fact]
        public async Task ManagementAPIManager_InsertPlayerScoreToReadModel_RecordInsertedSuccessfully()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            PlayerScorePublishedEvent domainEvent = TournamentTestData.GetPlayerScorePublishedEvent();

            await manager.InsertPlayerScoreToReadModel(domainEvent, CancellationToken.None);

            ManagementAPIReadModel verifyContext = this.GetContext(databaseName);
            verifyContext.PublishedPlayerScores.Count().ShouldBe(1);
        }

        [Fact]
        public async Task ManagementAPIManager_InsertPlayerScoreToReadModel_TournamentNotFound_ErrorThrown()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            Should.Throw<NotFoundException>(async () => { await manager.InsertPlayerScoreToReadModel(domainEvent, CancellationToken.None); });
        }

        [Fact]
        public async Task ManagementAPIManager_InsertPlayerSignUpForTournamentToReadModel_PlayerSignedUpEvent_NullEvent_ErrorThrown()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.InsertPlayerSignUpForTournamentToReadModel(domainEvent, CancellationToken.None); });
        }

        [Fact]
        public async Task ManagementAPIManager_InsertPlayerSignUpForTournamentToReadModel_PlayerSignedUpEvent_RecordUpdatedSuccessfully()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            await manager.InsertPlayerSignUpForTournamentToReadModel(domainEvent, CancellationToken.None);

            ManagementAPIReadModel verifyContext = this.GetContext(databaseName);
            verifyContext.PlayerTournamentSignUps.Count().ShouldBe(1);
            verifyContext.PlayerTournamentSignUps.First().TournamentId.ShouldBe(domainEvent.AggregateId);
            verifyContext.PlayerTournamentSignUps.First().PlayerId.ShouldBe(domainEvent.PlayerId);
        }

        [Fact]
        public async Task ManagementAPIManager_InsertPlayerSignUpForTournamentToReadModel_PlayerSignedUpEvent_TournamentNotFound_ErrorThrown()
        {
            String databaseName = Guid.NewGuid().ToString("N");
            Mock<IAggregateRepository<GolfClubAggregate>> clubRepository = new Mock<IAggregateRepository<GolfClubAggregate>>();
            ManagementAPIReadModel context = this.GetContext(databaseName);
            ...
            Should.Throw<NotFoundException>(async () => { await manager.InsertPlayerSignUpForTournamentToReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertTournamentToReadModel_GolfClubNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.InsertTournamentToReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertTournamentToReadModel_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.InsertTournamentToReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_InsertUserRecordToReadModel_GolfClubAdministrator_DuplicateRecord_ErrorThrown()
        {
            ...
            Mock<IAggregateRepository<PlayerAggregate>> playerRepository = new Mock<IAggregateRepository<PlayerAggregate>>();
            Mock<ISecurityService> securityService = new Mock<ISecurityService>();
            securityService.Setup(s => s.GetUserById(It.IsAny<Guid>(), CancellationToken.None))
                           .ReturnsAsync(GolfClubTestData.GetClubAdministratorUserResponse(GolfClubTestData.CreateMatchSecretaryRequest.GivenName,
        ...
        public async Task ManagementAPIManager_InsertUserRecordToReadModel_GolfClubAdministrator_RecordInsertedSuccessfully(String givenName,
                                                                                                                            String middleName,
                                                                                                                            String familyName)
        {
            String databaseName = Guid.NewGuid().ToString("N");
            ...
            Mock<ISecurityService> securityService = new Mock<ISecurityService>();
            securityService.Setup(s => s.GetUserById(It.IsAny<Guid>(), CancellationToken.None))
                           .ReturnsAsync(GolfClubTestData.GetClubAdministratorUserResponse(givenName, middleName, familyName));
            Mock<IAggregateRepository<GolfClubMembershipAggregate>> golfClubMembershipRepository = new Mock<IAggregateRepository<GolfClubMembershipAggregate>>();
        ...
        public async Task ManagementAPIManager_InsertUserRecordToReadModel_MatchSecretary_DuplicateRecord_ErrorThrown()
        {
            ...
            Mock<ISecurityService> securityService = new Mock<ISecurityService>();
            securityService.Setup(s => s.GetUserById(It.IsAny<Guid>(), CancellationToken.None))
                           .ReturnsAsync(GolfClubTestData.GetMatchSecretaryUserResponse(GolfClubTestData.CreateMatchSecretaryRequest.GivenName,
                                                                                        GolfClubTestData.CreateMatchSecretaryRequest.MiddleName,
                                                                                        GolfClubTestData.CreateMatchSecretaryRequest.FamilyName));
            Mock<IAggregateRepository<GolfClubMembershipAggregate>> golfClubMembershipRepository = new Mock<IAggregateRepository<GolfClubMembershipAggregate>>();
        ...
        [InlineData("givenName", "middleName", "")]
        [InlineData("givenName", "middleName", null)]
        public async Task ManagementAPIManager_InsertUserRecordToReadModel_MatchSecretary_RecordInsertedSuccessfully(String givenName,
                                                                                                                     String middleName,
                                                                                                                     String familyName)
        {
            String databaseName = Guid.NewGuid().ToString("N");
            ...
            Mock<ISecurityService> securityService = new Mock<ISecurityService>();
            securityService.Setup(s => s.GetUserById(It.IsAny<Guid>(), CancellationToken.None))
                           .ReturnsAsync(GolfClubTestData.GetMatchSecretaryUserResponse(givenName, middleName, familyName));
            Mock<IAggregateRepository<GolfClubMembershipAggregate>> golfClubMembershipRepository = new Mock<IAggregateRepository<GolfClubMembershipAggregate>>();
        ...
        public async Task ManagementAPIManager_UpdatePlayerHandicapRecordToReporting_MultipleRecords_RecordsUpdatedSuccessfully()
        {
            ...
            Should.NotThrow(async () => { await manager.UpdatePlayerHandicapRecordToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdatePlayerHandicapRecordToReporting_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.UpdatePlayerHandicapRecordToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdatePlayerHandicapRecordToReporting_PlayerAggregateNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.UpdatePlayerHandicapRecordToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdatePlayerMembershipToReporting_MultipleRecords_RecordsUpdatedSuccessfully()
        {
            ...
            Should.NotThrow(async () => { await manager.UpdatePlayerMembershipToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdatePlayerMembershipToReporting_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.UpdatePlayerMembershipToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdatePlayerMembershipToReporting_PlayerAggregateNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.UpdatePlayerMembershipToReporting(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdateTournamentInReadModel_PlayerScoreRecordedEvent_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.UpdateTournamentInReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdateTournamentInReadModel_PlayerScoreRecordedEvent_TournamentNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.UpdateTournamentInReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdateTournamentStatusInReadModel_TournamentCancelledEvent_NullEvent_ErrorThrown()
        {
            ...
            Should.Throw<ArgumentNullException>(async () => { await manager.UpdateTournamentStatusInReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagementAPIManager_UpdateTournamentStatusInReadModel_TournamentCancelledEvent_TournamentNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.UpdateTournamentStatusInReadModel(domainEvent, CancellationToken.None); });
        }
        ...
        public async Task ManagmentAPIManager_GetPlayer_PlayerNotFound_ErrorThrown()
        {
            ...
            Should.Throw<NotFoundException>(async () => { await manager.GetPlayerDetails(PlayerTestData.AggregateId, CancellationToken.None); });
        }
