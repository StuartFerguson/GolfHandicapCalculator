...
namespace ManagementAPI.Service.Tests
{
    public class ExceptionHandlerMiddewareUnitTest
    {
        ...
        private static ExceptionHandlerMiddleware.ErrorResponse GetErrorResponse(DefaultHttpContext context)
        {
            context.Response.Body.Seek(0, SeekOrigin.Begin);
            var reader = new StreamReader(context.Response.Body);
            var streamText = reader.ReadToEnd();
            var responseData = JsonConvert.DeserializeObject<ExceptionHandlerMiddleware.ErrorResponse>(streamText);
            return responseData;
        }
